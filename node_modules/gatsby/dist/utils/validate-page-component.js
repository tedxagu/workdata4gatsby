...
function validatePageComponent(page, directory, pluginName) {
  ...
  if (!component.includes(`/.cache/`) && process.env.NODE_ENV === `production`) {
    ...
    const includesDefaultExport = fileContent.includes(`export default`) || fileContent.includes(`module.exports`) || fileContent.includes(`exports.default`) || fileContent.includes(`exports["default"]`) || fileContent.match(/export \{.* as default.*\}/s) || // this check only applies to js and ts, not mdx
    /\.(jsx?|tsx?)/.test(_path.default.extname(component));
